...
def bar_plot(df, col=None, ylabel=None):
    ...
    ax.tick_params(left=False, bottom=False, colors='#525661', labelsize=14, fontproperties=font)

    plt.subplots_adjust(right=0.85)
    labels = ['Retail', 'Membership']
    ax.legend(labels, loc='center left', bbox_to_anchor=(1.02, 0.5), fontsize=14, fontproperties=font)
...
def package_distribution_plot(df, words, col, title, threshold=0):
    ...
    label = ax.annotate(title, color='#003264', fontsize=12, ha='center', va='center', xy=(0, 0), weight='bold', fontproperties=font)

    # Add a legend
    legend = ax.legend(patches, labels=[name for _, name in df_grouped.index], 
                       loc="lower center", bbox_to_anchor=(0.5, -0.25), 
                       title=None, title_fontsize=12, prop={'family': font})
...
def monthly_package_distribution_plot(df, words, col=None, threshold=0):
    ...
    ax.set_xticks(date_range)
    ax.set_xticklabels([dt.strftime('%b %Y') if dt.month == 1 else dt.strftime('%b') for dt in date_range], fontproperties=font)
    ...
    legend = plt.legend(name_order, bbox_to_anchor=(1.02, 0.5), loc='center left', fontsize=14)
    plt.tight_layout()
